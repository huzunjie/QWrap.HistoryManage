	...
	mix(HistoryManage.prototype,{
		...
		_init:function(){
			...
			*/
			...
			//change事件监听方法
			this.changeHandler && _t.on("change",this.changeHandler);
		...
		getHashParams:function(key){
			return queryUrl(this.getHashStr(),key);
		},
		...
		*/
		setHashParams:function(key,val){
			var _t = this, params = _t.getHashParams();
			loca.hash = isPlainObject(key)?
							encodeURIJson(val?key:mix(params,key,true))
							:key==undefined?
								""
								:val==undefined?
									encodeURIComponent(key)
									:encodeURIJson(setObject(params,key,val));
			return _t;
		},

		//取得get参数,输出为JSON对象(不支持pushState 所以urlParams不提供set方法)
		getUrlParams:function(paramKey){
			return queryUrl(loca.href,paramKey);
		},
		...
		start:function(){
            ...
            if ('onhashchange' in window) {
            	_t._timerHandler = function() {
            		_t.fire("change");
            	};
                on(window, 'hashchange', _t._timerHandler);
            } else {
            	_t._oldHashName = loca.hash;;
                (function() { //针对不支持onhashchange的浏览器用于hash状态检测，这里采用setTimeout有待优化
                    (_t._oldHashName != loca.hash) && _t.fire("change");
